...
class PenalizedPlayerPvp {
    ...
    // --------------------------------------------
    PenalizedPlayerPvp(string username, string hash, string ip, string sid, int id) {
        m_username = username;
        m_hash = hash;
        m_ip = ip;
        m_sid = sid;
...
// --------------------------------------------
class PenalizedPlayerBucketPvp {
    protected PenaltyManagerPvp@ m_manager;
    ...
    // --------------------------------------------
    PenalizedPlayerBucketPvp(PenaltyManagerPvp@ manager, string name, float updateTime, float timeFactor = 1.0f) {
        @m_manager = @manager;
        m_name = name;
    ...
    // --------------------------------------------
    PenalizedPlayerPvp@ get(string key) const {
        PenalizedPlayerPvp@ player;
        m_players.get(key, @player);
        return player;
    }

    // --------------------------------------------
    void add(PenalizedPlayerPvp@ player) {
        _log("PenaltyManagerPvp, " + m_name + ": add, player=" + player.m_username + ", hash=" + player.m_hash + ", count before=" + m_players.size() + ", sid=" + player.m_sid);
        m_players.set(player.m_sid, @player);
    }

    // --------------------------------------------
    void remove(PenalizedPlayerPvp@ player) {
        int s = size();
        m_players.erase(player.m_sid);
        if (size() != s) {
            _log("PenaltyManagerPvp, " + m_name + ": remove, player=" + player.m_username + ", sid=" + player.m_sid);
        }
    }
    ...
    void update(float time) {
        ...
        if (m_updateTimer < 0.0) {
            ...
            for (uint i = 0; i < keys.size(); ++i) {
                string key = keys[i];
                PenalizedPlayerPvp@ player = get(key);
    ...
    void addPlayersToSave(XmlElement@ root) {
        for (uint i = 0; i < m_players.getKeys().size(); ++i) {
            ...
            PenalizedPlayerPvp@ player = get(sid);
    ...
    // --------------------------------------------
    protected bool updatePlayer(PenalizedPlayerPvp@ player, float time) {
        bool ended = false;
...
// --------------------------------------------
class PenaltyManagerPvp : Tracker {
    ...
    PenalizedPlayerBucketPvp@ m_trackedPlayers;
    PenalizedPlayerBucketPvp@ m_persistentTrackedPlayers;
    ...
    // --------------------------------------------
    PenaltyManagerPvp(Metagame@ metagame, PlayerScore@ playerScore, int teamKillsToStartPenalty = 3, float penaltyTime = 1800.0, float forgiveTeamKillTime = 900.0, float persistentPenaltyTime = 1209600.0f) {
        @m_metagame = @metagame;
        ...
        @m_trackedPlayers = PenalizedPlayerBucketPvp(this, "tracked", 5.0);
        @m_persistentTrackedPlayers = PenalizedPlayerBucketPvp(this, "persistent", 60, persistentPenaltyTimeFactor);
        ...
        if (killer !is null) {
            ...
            if (target !is null) {
                ...
                if (killerFactionId == targetFactionId) {
                    ...
                    //if (true) {
                        ...
                        if (key != "ID0") {
                            if (m_trackedPlayers.exists(key)) {
                                ...
                                @player = PenalizedPlayerPvp(killerUsername, killerHash, killerIp, killerSid, killerId);
                    ...
                    // 玩家加分 被杀玩家增加死亡数
                    string killerUsername = killer.getStringAttribute("name");
                    m_playerScore.addPlayerKill(killerUsername, killerFactionId);
                    string targetUsername = target.getStringAttribute("name");
                    m_playerScore.addPlayerDie(targetUsername);
                    sendFactionMessage(m_metagame, 0, "当前比分：" + m_playerScore.getScoreCondition());
                    sendFactionMessage(m_metagame, 1, "当前比分：" + m_playerScore.getScoreCondition());
                }
            }
        }
    }
    ...
    protected void handlePlayerDisconnectEvent(const XmlElement@ event) {
        ...
        m_playerScore.resetPlayer(player.getStringAttribute("name"));
    }

    // ----------------------------------------------------
    protected void handlePlayerDisconnect(string key) {
        if (m_trackedPlayers.exists(key)) {
            PenalizedPlayerPvp@ p = m_trackedPlayers.get(key);
            _log("PenaltyManagerPvp: penalized/tracked player disconnected, player=" + p.m_username);
    ...
    protected void handlePlayerConnectEvent(const XmlElement@ event) {
        ...
        if (player !is null) {
            ...
            string key = player.getStringAttribute("sid");
            PenalizedPlayerPvp@ trackedPlayer;
            if (key != "ID0") {
                if (m_persistentTrackedPlayers.exists(key)) {
                    @trackedPlayer = m_persistentTrackedPlayers.get(key);

                    _log("PenaltyManagerPvp: penalized/tracked player connected, player=" + trackedPlayer.m_username + ", hash=" + hash + ", sid=" + trackedPlayer.m_sid);
            ...
            m_playerScore.initPlayer(player.getStringAttribute("name"), factionId);

            // 通知玩家现在的情况
            // 获取玩家faction
            sendPrivateMessage(m_metagame, playerId, "当前是PVP模式，你是 " + m_playerScore.getFactionName(factionId) + " 阵营");
            sendPrivateMessage(m_metagame, playerId, "当前比分：" + m_playerScore.getScoreCondition());
            int playerKill = m_playerScore.getPlayerKill(player.getStringAttribute("name"));
            int playerDie = m_playerScore.getPlayerDie(player.getStringAttribute("name"));
            sendPrivateMessage(m_metagame, playerId, "你的分数：" + playerKill + "， 你的死亡数：" + playerDie);
        }
    }

    // --------------------------------------------
    protected void addTeamKill(PenalizedPlayerPvp@ player) {
        ...
        player.m_teamKills += 1;
        _log("PenaltyManagerPvp: add_team_kill, player=" + player.m_username + ", team kills now = " + player.m_teamKills + ", sid=" + player.m_sid);
    ...
    // --------------------------------------------
    void startPenalty(PenalizedPlayerPvp@ player) {
        if (player.m_id >= 0) {
            // player is in-game
            _log("PenaltyManagerPvp: start_penalty, player=" + player.m_username + ", sid=" + player.m_sid);
    ...
    // --------------------------------------------
    void endPenalty(PenalizedPlayerPvp@ player) {
        if (player.m_id >= 0) {
            // player is in-game
            _log("PenaltyManagerPvp: end_penalty, player=" + player.m_username + ", sid=" + player.m_sid);
    ...
    // --------------------------------------------
    void removeTracking(PenalizedPlayerPvp@ player) {
    ...
    protected void save() {
        ...
        _log("PenaltyManagerPvp: save, players " + m_persistentTrackedPlayers.size() + " saved");
    }
    ...
    protected void load() {
        ...
        if (doc !is null) {
            ...
            if (root !is null) {
                ...
                for (uint i = 0; i < penalizedPlayers.size(); ++i) {
                    ...
                    PenalizedPlayerPvp player(username, hash, ip, sid, -1);
        ...
        _log("PenaltyManagerPvp: load, players " + m_persistentTrackedPlayers.size() + " loaded");
    }
